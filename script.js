    ...
    for (let i = 0; i < 4; ++i) {
      for (let j = 0; j < 4; ++j) {
        ...
        if ((cells[i][j].neighbours[keyCode] != undefined) && (flag == 0)) {
          if ((cells[i][j].neighbours[keyCode].value === null) && (flag == 0)) {
            updateMoves();
            let tempNumber = cells[i][j].number.innerHTML;
            let tempValue = cells[i][j].value;
            cells[i][j].number.innerHTML = cells[i][j].neighbours[keyCode].number.innerHTML;
            cells[i][j].value = cells[i][j].neighbours[keyCode].value;
            cells[i][j].neighbours[keyCode].number.innerHTML = tempNumber;
...
getNeighbours = (cells, cell) => {
  try {
    cell.neighbours[0] = cells[Math.floor(cell.index / 4)][cell.index % 4 - 1];
  } catch {
    cell.neighbours[0] = undefined;
  }
  try {
    cell.neighbours[1] = cells[Math.floor(cell.index / 4) - 1][cell.index % 4];
  } catch {
    cell.neighbours[1] = undefined;
  }
  try {
    cell.neighbours[2] = cells[Math.floor(cell.index / 4)][cell.index % 4 + 1];
  } catch {
    cell.neighbours[2] = undefined;
  }
  try {
    cell.neighbours[3] = cells[Math.floor(cell.index / 4) + 1][cell.index % 4];
  } catch {
...
for (let i = 0; i < 4; ++i) {
  for (let j = 0; j < 4; ++j) {
    cells[i].push(new Cell(documentCells[i * 4 + j], documentNumbers[i * 4 + j], i * 4 + j));
  }
}
...
for (let i = 0; i < newGameButtons.length; ++i) {
  newGameButtons[i].addEventListener("click", () => {generateCells(cells); closeModal();});
}
